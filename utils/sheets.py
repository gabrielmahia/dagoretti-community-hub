...
def append_row(tab_name: str, row_data: dict) -> bool:
    ...
    try:
        ...
        st.error(
            ...
            "Please email [contact@gabrielmahia.com](mailto:contact@gabrielmahia.com) directly."
        )
        return False
...
def not_configured_banner():
    st.warning(
        ...
        "[contact@gabrielmahia.com](mailto:contact@gabrielmahia.com) directly.",
        icon="⚠️",
    )
        ...
        if st.button("Submit correction", key=f"corr_btn_{key}"):
            if not correct or not source:
                ...
                if ok:
                    ...
                    st.info(f"Email correction to contact@gabrielmahia.com — Page: {page}, Field: {field}")
